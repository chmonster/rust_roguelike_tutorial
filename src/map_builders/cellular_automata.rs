...
impl MetaMapBuilder for CellularAutomataBuilder {
    ...
    fn build_map(&mut self, _rng: &mut rltk::RandomNumberGenerator, build_data: &mut BuilderMap) {
        console::log("CellularAutomata meta");
    ...
    fn apply_iteration(&mut self, build_data: &mut BuilderMap) {
        ...
        for y in 1..build_data.height - 1 {
            for x in 1..build_data.width - 1 {
                let idx = build_data.map.xy_idx(x, y);
                let mut neighbors = 0;
                if build_data.map.tiles[idx - 1] != TileType::Floor {
                    neighbors += 1;
                }
                if build_data.map.tiles[idx + 1] != TileType::Floor {
                    neighbors += 1;
                }
                if build_data.map.tiles[idx - build_data.width as usize] != TileType::Floor {
                    neighbors += 1;
                }
                if build_data.map.tiles[idx + build_data.width as usize] != TileType::Floor {
                    neighbors += 1;
                }
                if build_data.map.tiles[idx - (build_data.width as usize - 1)] != TileType::Floor {
                    neighbors += 1;
                }
                if build_data.map.tiles[idx - (build_data.width as usize + 1)] != TileType::Floor {
                    neighbors += 1;
