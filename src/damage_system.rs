...
use super::{
    ...
    GameLog, InBackpack, LootTable, Map, Name, Player, Pools, Position, RunState,
};
use rltk::console;
use specs::prelude::*;
...
//         for (entity, mut stats) in (&entities, &mut stats).join() {
...
//             log.entries
//                 .push(format!("You gained {} XP and {} gp", xp_gain, gold_gain));
...
pub fn delete_the_dead(ecs: &mut World) {
    ...
    let mut to_spawn: Vec<(String, Position)> = Vec::new();
    {
        ...
        for victim in dead.iter() {
            ...
            if let Some(table) = loot_tables.get(*victim) {
                let drop_finder = crate::data::get_item_drop(
...

    {
        for drop in to_spawn.iter() {
            crate::data::spawn_named_item(
                &crate::data::DATA.lock().unwrap(),
                ecs,
                &drop.0,
                crate::data::SpawnType::AtPosition {
                    x: drop.1.x,
                    y: drop.1.y,
                },
            );
